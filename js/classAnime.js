...
export class AnimeMain {
  ...
  animateUpIn(elem, time) {
    animate(elem, {
      translateY: "0.5rem",
      scale: { from: 0, to: 1, ease: "inOut(3)", duration: time },
    });
  }
  animateDownOut(elem, time) {
    animate(elem, {
      translateY: "-1rem",
      opacity: { to: 0, ease: "inOut(3)", duration: time },
      scale: { to: 0, ease: "inOut(3)", duration: time },
    });
  }
